    ...
    fun getDoubles(
        ...
        do {
            for (i in 1..rounds) {
                ...
                val potentiallyRests = players.toMutableList()
                for (g in games) {
                    ...
                    potentiallyRests.removeIf {
                        it.id == g.player1LeftId || it.id == g.player2LeftId ||
                                it.id == g.player1RightId || it.id == g.player2RightId
                    }
                }

                if (bestPerformance.worstPerformance > worstPerformance) worstPerformance = bestPerformance.worstPerformance
                totalGames[i] = games
                rests[i] = potentiallyRests
            }
            ...
            var repeatRest = tryUntilNoMoreThanOneRest
            val restIds = rests.values.flatten()
            if (tryUntilNoMoreThanOneRest && restIds.size == restIds.toSet().size)
                repeatRest = false

            repeat = repeatPerformance || repeatRest
            counter += 1
        ...
        return Triple(totalGames.toMap(), rests.toMap(), worstPerformance)
    }
